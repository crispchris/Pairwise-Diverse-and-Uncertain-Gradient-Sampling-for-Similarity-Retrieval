...
Christoffer Löffler,  *Pairwise Diverse and Uncertain Gradient-Sampling for Similarity Retrieval*  
...
```
├── main.py                  # Main training and active learning loop (Hydra + MLflow)
├── model.py                 # Siamese network, custom embedding loss, and MAPE metric
├── data_module.py           # Torch Dataset and DataLoader utilities
├── cost_fns.py              # Hungarian and keypoint-based cost functions
├── retrieval_metrics.py     # Retrieval and ranking evaluation metrics (Recall/Precision, mAP, nDCG, MSRCC)
├── gradient_metrics.py      # Per-example gradient diagnostics
├── conf/
│   └── config.yaml          # Experiment configuration (Hydra)
```
...
Data come from **NBA SportVU** ([linouk23/NBA-Player-Movements](https://github.com/linouk23/NBA-Player-Movements)) and **NFL Next Gen Stats** ([asonty/ngs_highlights](https://github.com/asonty/ngs_highlights)).  
Each pipeline converts raw tracking data into fixed‑length event tensors, then builds pairwise samples with Hungarian alignment costs.

For basketball, raw SportVU JSON logs (compressed `.7z` archives) were parsed into per-play tensors of shape *(2, 11, 150)*, representing 11 tracked entities (10 players + ball) with x/y positions over 150 frames. Invalid or duplicate plays were filtered, and the remaining events were split into train/validation/test partitions.  

For football, NGS play-level TSV files were normalized into tensors of shape *(2, 23, 50)*, capturing 22 players + ball over 50 frames. Sliding windows and padding ensured consistent temporal coverage across plays.  

In both sports, **pairwise training samples** were generated by randomly selecting event pairs and computing **Hungarian alignment costs** between trajectories, producing triplets *(i, j, cost)*. These were serialized into `.pt` tensors (`pairs_train.pt`, `pairs_val.pt`, `pairs_test.pt`) used for similarity learning and retrieval evaluation.
...
1. **Convert raw JSON logs**  
   ...
   ```  
   → Produces `nba_sportvu_train.pt`, `nba_sportvu_val.pt`, `nba_sportvu_test.pt` (events of shape `(2, 11, 150)`).

2. **Sample event pairs**  
   ```bash
   python 02_nba_sample_pairs.py
   ```  
   → Generates `pairs_train_10M.pt`, `pairs_val_10M.pt`, `pairs_test_10M.pt` with `(i, j, cost)` triplets.
...
1. **Convert raw TSV plays**  
   ...
   ```  
   → Produces `nfl_ngs_train.pt`, `nfl_ngs_val.pt`, `nfl_ngs_test.pt` (events of shape `(2, 23, 50)`).

2. **Sample event pairs**  
   ```bash
   python 02_nfl_sample_pairs.py
   ```  
   → Outputs `pairs_nfl_train_1M.pt`, `pairs_nfl_val_1M.pt`, `pairs_nfl_test_1M.pt`.
